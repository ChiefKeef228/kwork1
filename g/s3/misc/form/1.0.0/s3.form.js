			...
			$(parent).each(function() {
				...
				function res(response) {
					if (response.result.success && response.result['redirect_url']) {
						...
						if ($captcha.length) {
							$.getScript('../captcha.oml.ru/static/captchac81e.html?2', function() {
								var $d = $container.find("[id^=s3_captcha_cn]");
								mgCaptcha.draw("my/s3/captcha/get.html", ($d.length ? $d.get(0) : null));
							});
						}
						...
						if ($uploads.length) {
							if (typeof window['newSWFU'] !== 'function') {
								$.getScript('shared/s3/swfupload/swfupload.upload.pack.html', function() {
									inituploaders();
								});
							} else {
								inituploaders();
